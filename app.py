...
# Custom CSS untuk kartu metrik berwarna (Sesuai gambar pertama)
st.markdown("""
    <style>
...
try:
    ...
    # Pencarian (Search Nama Barang, Kode Item, No PO)
    ...
    # Filter Tanggal (Start Date to End Date)
    ...
    if search_query:
        # Search di beberapa kolom sekaligus
    ...
    # --- 5. TAMPILAN MATRIK UTAMA (DINAMIS) ---
    ...
    # --- 6. GRAFIK TIMELINE BULANAN (PO VS RECEIVING) ---
    ...
    if len(selected_event.selection.rows) > 0:
        ...
        st.dataframe(df_detail[available], use_container_width=True)
